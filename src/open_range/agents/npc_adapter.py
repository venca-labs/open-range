...
def configure_npc_llm(
    ...
    *,
    ...
    resolved_model = model or os.environ.get("MODEL_ID") or "moonshotai/kimi-k2-instruct"
    ...
    # Directly inject the model fallback natively into the TinyTroupe global state
    # to avoid brittle dependency on the filesystem config.ini file across deployments
    import tinytroupe
    if "OpenAI" in tinytroupe.config:
        tinytroupe.config["OpenAI"]["model"] = resolved_model
        # Safely enforce scaling boundaries dynamically without strict .ini dependence
        tinytroupe.config["OpenAI"]["max_completion_tokens"] = "4096"
        tinytroupe.config["OpenAI"]["max_concurrent_model_calls"] = "4"
        tinytroupe.config["OpenAI"]["timeout"] = "480"
    
    # Disable semantic consolidations unconditionally to prevent OpenAI embedding API 401 crashes footprint
    if "Cognition" in tinytroupe.config:
        tinytroupe.config["Cognition"]["enable_memory_consolidation"] = "False"
        tinytroupe.config["Cognition"]["enable_continuous_contextual_semantic_memory_retrieval"] = "False"
