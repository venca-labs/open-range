"""Autonomous Agent-backed green scheduler for executing emergent autonomous behaviors."""
...
class AutonomousGreenScheduler(GreenScheduler):
    """An autonomous green scheduler backed by logical autonomous agents and Kimi K2.

    This maps the time-stepping runtime loop directly to the Autonomous World
    ...
    def __init__(
        ...
        self._autonomous_world: Any | None = None
        self._autonomous_agents: dict[str, Any] = {}
        self._event_buffer: list[RuntimeEvent] = []

        try:
            configure_autonomous_llm(api_key=api_key, model=model, base_url=base_url)
            self._available = True
        except ImportError:
            self._available = False
            logger.warning(
                "Autonomous framework dependencies not installed. AutonomousGreenScheduler will be a no-op."
            )
    ...
    def reset(self, snapshot: RuntimeSnapshot, episode_config: EpisodeConfig) -> None:
        ...
        # Initialize autonomous agents from personas
        self._autonomous_agents = {}
        for p in personas:
            agent = persona_to_autonomous(p)
            self._autonomous_agents[p.id] = agent

        # Create the simulation environment
        company_name = getattr(snapshot.world, "company_name", "enterprise")
        self._autonomous_world = create_enterprise_world(
            company_name, list(self._autonomous_agents.values())
        )

    def record_event(self, event: RuntimeEvent) -> None:
        if not self._autonomous_world or not self._episode_config.green_branch_enabled:
            return
    ...
    def advance_until(self, sim_time: float) -> None:
        ...
        # Push ONLY relevant observations natively to the Autonomous World
        if self._event_buffer:
            ...
            if anomalies:
                ...
                self._autonomous_world.broadcast(
        ...
        self._autonomous_world.run(steps=steps_to_run)

        # Extract actions from agents' recent memory buffer directly via episodic memory
        for agent_id, agent in self._autonomous_agents.items():
