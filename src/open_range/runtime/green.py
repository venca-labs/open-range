...
class ScriptedGreenScheduler:
    ...
    def reset(self, snapshot: RuntimeSnapshot, episode_config: EpisodeConfig) -> None:
        ...
        self._security_persona_ids: set[str] = {
            p.id
            for p in snapshot.world.green_personas
    ...
    def record_event(self, event: RuntimeEvent) -> None:
        ...
        security_personas = [
            p for p in all_personas if p.id in self._security_persona_ids
        ]
    ...
    def _routine_actions(self, slot: int) -> tuple[Action, ...]:
        ...
        chosen = due if max_parallel >= len(due) else rng.sample(due, k=max_parallel)
        ...
        for persona in chosen:
            ...
            if inbox:
                ...
                if routine == "idle":
                    self._next_action_slot[persona.id] = slot + rng.randint(
                        _MIN_CADENCE, _MAX_CADENCE
                    )
                    continue
    ...
    def _pick_recipient(
        ...
        if ms is not None:
            recent_recipients = {m.object_ for m in ms.recent(5) if "sent" in m.tags}
    ...
    def _susceptibility_score(
        self, persona: GreenPersona, event: RuntimeEvent
    ) -> float:
        ...
        if not persona.susceptibility:
            ...
            base = persona.susceptibility.get(
                event_key, max(persona.susceptibility.values())
            )
...
def _planner_hint_for_slot(planner: DailyPlanner, slot: int) -> ScheduledAction | None:
