...
from open_range.builder.npc.planner import DailyPlanner, _parse_json
...
class LLMNPCAgent:
    ...
    async def decide(self, persona: GreenPersona, stimulus: Stimulus) -> NPCAction:
        ...
        importance = (
            8.0
            if result.action in ("report_to_IT", "share_credentials", "click_link")
            else 4.0
        )
        self._memory.add(
            subject=persona.id,
    ...
    async def run_loop(
        ...
        base_interval = (
            getattr(getattr(snapshot, "npc_traffic", None), "action_interval_min", None)
            or 2
        )
            ...
            try:
                ...
                if self._memory.needs_reflection():
                    ...
                    if reflections:
                        ...
                        logger.debug(
                            "NPC %s reflected: %d insights",
                            persona.id,
                            len(reflections),
                        )
                    await self._planner.plan_day(
                        persona, env_context, reflections=reflections
                    )
                ...
                routine = await self.next_routine_action(
                    persona, env_context, plan_hint=plan_hint
                )
                ...
                try:
                    ...
                    if mail_output and mail_output.strip():
                        ...
                        content = await containers.exec(
                            "mail",
                            f"head -50 '{email_file}' 2>/dev/null || true",
                        )
    ...
    actions: list[dict[str, str]] = [
        {
            ...
            "detail": "Checking portal",
            "email_body": "",
        },
        {
            "action": "idle",
            "target": "",
            "detail": "Reading documents at desk",
            "email_body": "",
        },
    ]
    if shares:
...
class RuleBasedNPCBehavior:
    ...
    async def decide(self, persona: GreenPersona, stimulus: Stimulus) -> NPCAction:
        ...
        if persona.awareness > 0.7 and score < 0.8:
            return NPCAction(
                action="report_to_IT", side_effects=["reported suspicious email to IT"]
            )
        elif score > 0.6:
            return NPCAction(
                action="click_link", side_effects=["clicked link in email"]
            )
        elif score > 0.3:
            return NPCAction(action="ignore")
        else:
            return NPCAction(
                action="report_to_IT", side_effects=["forwarded to security team"]
            )
