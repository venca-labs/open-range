...
_ROUTINE_SERVICE: dict[str, str] = {
    ...
    "login": "svc-web",
    "lookup": "svc-web",
    "send_email": "svc-email",
    "send_mail": "svc-email",
    "send_chat": "svc-chat",
    "read_mail": "svc-email",
    "read_chat": "svc-chat",
    "read_message": "svc-email",
    "access_share": "svc-fileshare",
    "query_db": "svc-db",
}
...
class RuntimeNPCAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self._task_idx = 0
        stable_seed = int(
            hashlib.md5(persona.id.encode("utf-8")).hexdigest()[:8],
            16,
        )
    ...
    async def run(self) -> None:
        ...
        logger.info(
            "NPC %s (%s) starting workday — %d tasks, llm=%s",
            self.persona.id,
            self.persona.role,
            len(self.tasks),
            bool(self.model),
        ...
        try:
            ...
            logger.info(
                "NPC %s workday ended (%d actions)",
                self.persona.id,
                self._actions_submitted,
            )
        ...
        for obs in self._pending_observations:
            ...
            if obs.category in ("message", "alert"):
                ...
                if reaction.payload.get("routine") == "read_mail":
                    ...
                    reply_modality = reaction.payload.get("modality", "email")
                    if sender_id and not subject.startswith("Re:"):
                        reply = await self._reply_action_async(
                            sender_id, modality=reply_modality
                        )
                        if reply is not None:
                            self._reaction_queue.append(reply)
            else:
    ...
    def _service_issue_action(self, event: RuntimeEvent) -> Action:
        ...
        self.memory.add(
            subject=self.persona.id,
            relation="noticed_service_outage",
            object_=affected_service,
            importance=4.0,
    ...
    def _reply_action(self, sender_id: str, modality: str = "email") -> Action | None:
        ...
        reply = generate_reply_content(
            replier_role=self.persona.role,
            replier_name=self.persona.id,
    ...
    async def _reply_action_async(
        self, sender_id: str, modality: str = "email"
    ) -> Action | None:
    ...
    def _finish_reply(
        self, sender_id: str, subject: str, body: str, modality: str = "email"
    ) -> Action:
        ...
        if self.mail_store:
            self.mail_store.deliver(
                self.persona.id, sender_id, subject, body, modality=modality
            )
        service = _modality_service(modality)
        self.memory.add(
            subject=self.persona.id,
            relation="replied_to",
            object_=f"{sender_id}:{subject}",
            importance=3.5,
    ...
    async def _maybe_submit_next_async(self) -> None:
        ...
        if (
            self._use_llm
            and task.needs_llm
            and task.action in ("send_email", "send_mail")
        ):
    ...
    def _maybe_social_message(self) -> NPCTask | None:
        ...
        modality = (
            self._preferred_modality if self._preferred_modality == "chat" else "chat"
        )
        ...
        subject = (
            "" if modality == "chat" else social["subject"].format(name=friend_name)
        )
        return NPCTask(
    ...
    def _build_action(self, task: NPCTask) -> Action:
        ...
        service = (
            _modality_service(modality)
            if is_message
            else _ROUTINE_SERVICE.get(task.action, "svc-web")
        )
        ...
        return Action(
            actor_id=self.persona.id, role="green", kind=kind, payload=payload
        )
    ...
    async def _build_action_llm(self, task: NPCTask) -> Action:
        ...
        try:
            ...
            logger.debug(
                "NPC %s email LLM failed, using template: %s", self.persona.id, exc
            )
        ...
        if self.mail_store and (subject or body):
            self.mail_store.deliver(
                self.persona.id, recipient_id, subject, body, modality=modality
            )
        return Action(
            actor_id=self.persona.id, role="green", kind="mail", payload=payload
        )
    ...
    async def _llm_compose_reply(
        ...
        sender_persona = next((c for c in self.colleagues if c.id == sender_id), None)
        sender_name = (
            display_name(sender_persona) if sender_persona else display_name(sender_id)
        )
    ...
    async def _llm_decide_reaction(self, event: RuntimeEvent) -> Action:
        ...
        try:
            ...
            logger.debug(
                "NPC %s LLM security decision: %s (%s)",
                self.persona.id,
                action_choice,
                raw.get("reason", ""),
            )
        ...
        if action_choice == "report_to_it" or action_choice == "report":
            ...
            self.memory.add(
                subject=self.persona.id,
                relation="ignored_security_event",
                object_=event.target_entity,
                importance=3.0,
...
_SOCIAL_MESSAGES = [
    {
        "subject": "lunch today?",
        "body": "Hey {name}, want to grab lunch? I'm thinking tacos.",
    },
    {
        "subject": "did you see this",
        "body": "Hey {name}, did you catch the all-hands yesterday? That Q2 roadmap slide was wild.",
    },
    {
        "subject": "weekend plans?",
        "body": "Hey {name}, any plans this weekend? I'm trying to get a group together for hiking.",
    },
    {
        "subject": "coffee run",
        "body": "Hey {name}, heading to grab coffee — want anything? The usual?",
    },
    {
        "subject": "random thought",
        "body": "Hey {name}, I just realized we've been in back-to-back meetings all week. We should block some focus time.",
    },
    {
        "subject": "that meeting tho",
        "body": "Hey {name}, was that meeting as painful for you as it was for me? I think we could have covered that in an email.",
    },
    {
        "subject": "heads up",
        "body": "Hey {name}, heard they might reorganize the floor plan again. Just a heads up in case you want to claim a good desk.",
    },
    {
        "subject": "nice work!",
        "body": "Hey {name}, saw your presentation deck — really well done. The data viz on slide 3 was great.",
    },
]
