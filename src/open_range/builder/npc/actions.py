...
_HTTP_STATUS_WEIGHTS = [
    (200, 0.75), (200, 0.05), (301, 0.05), (304, 0.08), (404, 0.05), (403, 0.02),
]
...
class NPCActionExecutor:
    ...
    async def _routine_email(self, persona, username, target, detail, body):
        ...
        return _chat_log(persona, recipient, detail or f"Message to {recipient}", f"mail:{username}")
    ...
    async def _routine_idle(self, persona, username, target, detail, _eb):
        return _log(persona, "idle", detail or "Away from desk", "none", log_type="system_activity")
    ...
    async def _react_share_creds(self, persona: NPCPersona, action: NPCAction) -> dict[str, Any]:
        ...
        await self.containers.exec(source, f"echo {safe_content} >> /tmp/leaked_{ts_i}.txt")
        ...
        return _se_log(persona, "share_credentials", f"{persona.id} leaked credentials", "web+siem", result="success")
