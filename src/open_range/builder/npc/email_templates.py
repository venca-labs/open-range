...
def generate_email_content(
    ...
    """
    rng = random.Random(
        int(hashlib.md5(f"{sender_name}:{recipient_name}:{seed}".encode()).hexdigest()[:8], 16)
    )
...
def generate_reply_content(
    ...
    """
    rng = random.Random(
        int(hashlib.md5(f"{replier_name}:reply:{original_sender}:{seed}".encode()).hexdigest()[:8], 16)
    )
