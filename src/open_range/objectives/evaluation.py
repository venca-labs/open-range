    ...
    *,
    ...
    if grader.grader_kind == "service_health":
        ...
        return health < 0.9 or any(
            event_type(event) == "ServiceDegraded" for event in matched_events
        )
    if grader.grader_kind == "file_exists":
        ...
        return has_target and bool(matched_events)
    if grader.grader_kind in {"db_row_read", "db_row_write"}:
        ...
        return grader.service_id in db_services and bool(matched_events)
...
def evaluate_red_objectives(
    *,
    ...
    for objective in objective_specs:
        ...
        if grader is None:
            if any(event_matches_objective(event, predicate) for event in event_list):
