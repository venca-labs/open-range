...
from open_range.builder.npc.outbox import (
    ActionOutbox,
    EventInbox,
    MessageStore,
    SimClock,
...
class TestObservationCreation:
    ...
    def test_own_event_is_none(self):
        ...
        agent, *_ = _make_agent(persona=bob)
        evt = _event(
            "own-1", actor="green", source_entity="bob.smith", target_entity="svc-web"
        )
        obs = agent._create_observation(evt)
        assert obs is None
...
class TestShouldCheckObservations:
    ...
    def test_age_boost_increases_probability(self):
        ...
        checks = sum(1 for _ in range(100) if agent._should_check_observations())
...
class TestSecurityFastPath:
    ...
    def test_suspicious_action_immediate(self):
        ...
        inbox.push(
            _event(
                "sec-2", event_type="SuspiciousActionObserved", source_entity="svc-siem"
            )
        )
        agent._process_inbox()
        assert len(agent._pending_observations) == 0
    ...
    def test_service_degraded_is_deferred(self):
        ...
        clock.advance(0.0)
        inbox.push(
            _event("sec-3", event_type="ServiceDegraded", target_entity="svc-web")
        )
        agent._process_inbox()
        assert len(agent._pending_observations) == 1
...
class TestBetweenTaskBatchCheck:
    ...
    def test_between_tasks_processes_pending_message(self):
        ...
        ms = MessageStore()
        ms.deliver(
            "janet.liu", focused.id, "Update", "Please review.", modality="email"
        )
        task = NPCTask("Browse portal", "browse", "/", "Checking", 0, 15)
        ...
        # Message arrives
        inbox.push(
            _event("bt-msg", source_entity="janet.liu", target_entity=focused.id)
        )
        agent._process_inbox()
        assert len(agent._pending_observations) == 1
...
class TestDoneProperty:
    def test_done_accounts_for_pending_observations(self):
        ...
        inbox.push(
            _event("done-1", source_entity="janet.liu", target_entity="bob.smith")
        )
        agent._process_inbox()
...
class TestProcessPendingObservations:
    def test_message_observation_produces_read_and_reply(self):
        ...
        ms = MessageStore()
        ms.deliver(
            "janet.liu", "bob.smith", "Q1 Report", "Please review.", modality="email"
        )
        agent, outbox, inbox, clock, _ = _make_agent(
            persona=bob,
            tasks=[],
        ...
        clock.advance(0.0)
        inbox.push(
            _event("proc-1", source_entity="janet.liu", target_entity="bob.smith")
        )
        agent._process_inbox()
        assert len(agent._pending_observations) == 1
        agent._process_pending_observations()
    ...
    def test_alert_observation_produces_reaction(self):
        ...
        clock.advance(0.0)
        inbox.push(
            _event("proc-2", event_type="ServiceDegraded", target_entity="svc-web")
        )
        agent._process_inbox()
        assert len(agent._pending_observations) == 1
        agent._process_pending_observations()
    ...
    def test_already_reacted_observations_skipped(self):
        ...
        clock.advance(0.0)
        inbox.push(
            _event("proc-3", source_entity="janet.liu", target_entity="bob.smith")
        )
        agent._process_inbox()
...
class TestAsyncParity:
    def test_async_inbox_defers_messages(self):
        ...
        clock.advance(0.0)
        inbox.push(
            _event("async-1", source_entity="janet.liu", target_entity="bob.smith")
        )
    ...
    def test_async_process_pending_produces_same_result(self):
        ...
        def _run_sync():
            ...
            clock.advance(0.0)
            inbox.push(
                _event("parity-s", source_entity="janet.liu", target_entity="bob.smith")
            )
            agent._process_inbox()
            agent._process_pending_observations()
        ...
        def _run_async():
            ...
            clock.advance(0.0)
            inbox.push(
                _event("parity-a", source_entity="janet.liu", target_entity="bob.smith")
            )
...
class TestDeterminism:
    def test_same_seed_same_check_decisions(self):
        ...
        def _run():
            ...
            clock.advance(0.0)
            inbox.push(
                _event("det-1", source_entity="janet.liu", target_entity="bob.smith")
            )
            agent._process_inbox()
            results = []
            for i in range(20):
