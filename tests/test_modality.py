...
from open_range.builder.npc.identity import NPCBackstory
...
class TestBuildActionModality:
    def test_chat_task_strips_subject(self):
        ...
        task = NPCTask(
            ...
            0,
            20,
            collaborators=(personas[0].id,),
            modality="chat",
            email_subject="Should be stripped",
            email_body="Chat body here.",
        )
    ...
    def test_chat_delivered_to_store_without_subject(self):
        ...
        task = NPCTask(
            ...
            0,
            20,
            collaborators=(target.id,),
            modality="chat",
            email_subject="Should vanish",
            email_body="Chat body.",
        )
...
class TestReplyOnSameMedium:
    ...
    def test_chat_reply_strips_subject(self):
        ...
        ms.deliver(
            "dan.wu", "bob.smith", "", "Hey, quick question about K8s", modality="chat"
        )
...
class TestSocialMessageModality:
    ...
    def test_social_message_uses_chat(self):
        ...
        agent, outbox, _, clock, _ = _make_agent(
            persona=janet,
            tasks=[NPCTask("Lunch", "idle", "", "Break", 0, 60)],
        )
        clock.advance(0.0)
...
class TestPreferredModality:
    ...
    def test_known_chat_personas(self):
        ...
        for p in _personas():
            if p.id in chat_ids:
                assert p.profile.backstory.preferred_modality == "chat", (
                    f"{p.id} should prefer chat"
                )
    ...
    def test_known_email_personas(self):
        ...
        for p in _personas():
            if p.id in email_ids:
                assert p.profile.backstory.preferred_modality == "email", (
                    f"{p.id} should prefer email"
                )
...
class TestObservabilitySurface:
    ...
    def test_chat_action_emits_on_svc_chat(self):
        from open_range.runtime.events import green_events_for_action
