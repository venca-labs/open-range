...
Marks
-----
live_model  -- requires a real model backend and makes live API calls.
              Set OPENRANGE_NPC_MODEL explicitly if you do not want the
              default OpenAI-compatible model path.
live_kind   -- requires a running Kind cluster (openrange context) with
              the or-llm-tier1-test release deployed.

Run all smoke tests:

    uv run -m pytest tests/test_npc_smoke.py -v -m "live_model or live_kind"

Run only the LLM smoke tests (no cluster needed):

    uv run -m pytest tests/test_npc_smoke.py -v -m live_model
"""
...
_NPC_MODEL = os.environ.get(
    "OPENRANGE_NPC_MODEL",
    os.environ.get("MODEL_ID", "moonshotai/kimi-k2-instruct"),
)
    ...
    try:
        result = subprocess.run(
            [
                ...
                "--all-namespaces",
                "-l",
                f"app.kubernetes.io/instance={_KIND_RELEASE}",
                "-o",
    ...
    try:
        result = subprocess.run(
            [
                ...
                "-o",
                "jsonpath={range .items[*]}{.metadata.namespace}{'/'}{.metadata.name}{'|'}{.metadata.labels.app}{'\\n'}{end}",
            ],
            capture_output=True,
            text=True,
            timeout=10,
...
@pytest.fixture
def pod_set():
    from open_range.render.live import PodSet
...
@pytest.mark.skipif(
    not _has_api_key(),
    reason="ANTHROPIC_API_KEY, OPENAI_API_KEY, or NVIDIA_API_KEY not set",
)
...
@pytest.mark.skipif(
    not _release_pods_running(), reason=f"{_KIND_RELEASE!r} pods not Running"
)
class TestNPCActionExecutorLive:
    ...
    def test_idle_action(self, executor, persona):
        ...
        log = asyncio.run(executor.execute_routine(persona, "idle", "", "Lunch break"))
    ...
    def test_share_access(self, executor, persona):
        ...
        log = asyncio.run(
            executor.execute_routine(
                persona, "access_share", "general", "Checking shared drive"
            )
        )
    ...
    def test_query_db(self, executor, it_persona):
        ...
        log = asyncio.run(
            executor.execute_routine(
                it_persona, "query_db", "", "Afternoon health check"
            )
        )
