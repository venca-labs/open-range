    ...
    def _decide(self, persona, plausibility):
        return _run(
            self.b.decide(
                persona,
                Stimulus(
                    "phishing_email", "evil@bad.com", "Win prize", "", plausibility
                ),
            )
        )
...
class TestReactiveExecutor:
    ...
    def test_react_click_extracts_url_from_side_effects(self):
        ...
        _run(
            executor.execute(
                ...
                NPCAction(
                    action="click_link",
                    side_effects=["clicked http://evil.example.com/payload"],
                ),
            )
        )
    ...
    def test_react_report_to_it_logs_blocked(self):
        ...
        result = _run(
            executor.execute(
                persona,
                NPCAction(
                    action="report_to_IT", side_effects=["suspicious email detected"]
                ),
            )
        )
        assert result["type"] == "social_engineering"
    ...
    def test_react_reply_logs_success(self):
        ...
        result = _run(
            executor.execute(
                persona, NPCAction(action="reply", response_content="Acknowledged")
            )
        )
        assert result["type"] == "social_engineering"
...
class TestRoutineExecutor:
    ...
    def test_browse_returns_web_request_log(self):
        ...
        persona = _make_persona()
        result = _run(
            executor.execute_routine(persona, "browse", "/index.php", "Browsing")
        )
    ...
    def test_send_email_returns_npc_chat_log(self):
        ...
        result = _run(
            executor.execute_routine(
                persona, "send_email", "bob", "Quick update", "Hi Bob"
            )
        )
    ...
    def test_lookup_returns_web_request_log(self):
        ...
        persona = _make_persona()
        result = _run(
            executor.execute_routine(persona, "lookup", "status", "Looking up")
        )
        assert result["type"] == "web_request"
    ...
    def test_login_returns_auth_log(self):
        ...
        persona = _make_persona()
        result = _run(
            executor.execute_routine(persona, "login", "/login.php", "Portal login")
        )
    ...
    def test_persona_id_present_in_all_log_types(self):
        ...
        for action in (
            ...
            assert result["persona"] == "janet.liu", (
                f"Missing persona for action={action}"
            )
