...
def _flag_value(snapshot: object) -> str:
    return next(
        n.attrs["value_ref"]
        for n in snapshot.world_graph.nodes  # type: ignore[attr-defined]
...
def test_v1_episode_round_trip_with_scripted_solve(tmp_path: Path) -> None:
    ...
    assert report.final_state["result"]["flag"] == flag
...
def test_v1_episode_runs_npcs_during_ticks(tmp_path: Path) -> None:
    ...
    try:
        ...
        report = service.stop_episode(handle)
    requests = report.final_state["requests"]
