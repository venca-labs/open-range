...
from open_range.builder.npc.npc_agent import (
    LLMNPCAgent,
    NPCAction,
    Stimulus,
)
from open_range.builder.npc.persona import default_personas
from open_range.builder.npc.planner import (
    DailyPlanner,
    ScheduledAction,
    _template_schedule,
)
...
class TestMemoryEntry:
    ...
    def test_relevance_empty_tags(self):
        entry = MemoryEntry(
            subject="x", relation="y", object_="z", importance=5.0, tags=[]
        )
    ...
    def test_relevance_entity_tokens(self):
        ...
        entry = MemoryEntry(
            subject="janet.liu",
            relation="received_phishing_from",
            object_="attacker@evil.com",
            importance=5.0,
        )
    ...
    def test_retrieval_score_combines_components(self):
        entry = MemoryEntry(
            subject="janet.liu",
            relation="reacted_to_phishing_with",
            object_="report_to_IT",
            importance=10.0,
            tags=["phishing"],
        )
...
class TestMemoryStream:
    ...
    def test_eviction_at_capacity(self):
        ...
        ms.add(
            "janet.liu", "new", "entry", importance=6.0
        )  # should evict low importance
        assert len(ms) == 3
...
class TestDailyPlanner:
    ...
    def test_plan_day_llm_success(self, mock_litellm):
        ...
        mock_response.choices[
            0
        ].message.content = '{"schedule": [{"hour": 9, "action": "browse", "target": "/", "detail": "Morning check"}], "mood": "focused", "focus": "project work"}'
...
class TestLLMNPCAgentMemory:
    ...
    def test_decide_records_memory(self):
        ...
        with patch.object(
            agent, "decide", new=AsyncMock(return_value=NPCAction(action="ignore"))
        ):
    ...
    def test_decide_passes_memory_context(self, mock_litellm):
        ...
        mock_response.choices[
            0
        ].message.content = '{"action": "report_to_IT", "response_content": "", "side_effects": ["reported"]}'
        mock_litellm.acompletion = AsyncMock(return_value=mock_response)
        ...
        persona = default_personas()[0]
        stimulus = Stimulus(
            type="email", sender="x", subject="y", content="z", plausibility=0.5
        )
    ...
    def test_decide_records_high_importance_for_report(self, mock_litellm):
        ...
        mock_response.choices[
            ...
            '{"action": "report_to_IT", "response_content": "", "side_effects": []}'
        )
        mock_litellm.acompletion = AsyncMock(return_value=mock_response)
    ...
    def test_next_routine_action_passes_memory_and_hint(self, mock_litellm):
        ...
        mock_response.choices[
            0
        ].message.content = '{"action": "browse", "target": "/", "detail": "morning check", "email_body": ""}'
        mock_litellm.acompletion = AsyncMock(return_value=mock_response)

        agent = self._make_agent()
        agent._memory.add(
            "janet.liu", "browsed", "/dashboard", importance=2.0, tags=["browse"]
        )
        persona = default_personas()[0]
        env_context = {
            "pages": ["/", "/status"],
        ...
        hint = {
            "action": "browse",
            "target": "/status",
            "detail": "Check service status",
        }

        result = asyncio.run(
            agent.next_routine_action(persona, env_context, plan_hint=hint)
        )
    ...
    def test_next_routine_records_memory(self, mock_litellm):
        ...
        mock_response.choices[
            0
        ].message.content = '{"action": "idle", "target": "", "detail": "Taking a break", "email_body": ""}'
        mock_litellm.acompletion = AsyncMock(return_value=mock_response)
...
class TestNPCManagerIntegration:
    ...
    def test_action_log_types(self):
        ...
        persona = default_personas()[0]
        assert (
            _web_log(persona, "browse", "detail", "/foo", "UA")["type"] == "web_request"
        )
...
class TestPerformanceBenchmark:
    ...
    def test_eight_npcs_twelve_ticks_under_30s(self):
        ...
        async def _run_npc(persona: GreenPersona) -> LLMNPCAgent:
            ...
            with (
                patch("open_range.builder.npc.npc_agent.litellm") as mock_llm,
                ...
                mock_planner_llm.acompletion = AsyncMock(
                ...
                for _ in range(12):
                    ...
                    routine = await agent.next_routine_action(
                        persona, env_ctx, plan_hint=plan_hint
                    )
